...
ggplot(vector_data, aes(x = x, y = y)) + 
  geom_segment(aes(xend = xend, yend = yend), arrow = arrow(length = unit(0.3, "cm"))) + 
  xlim(-2, 6) + 
  ylim(-2, 4) +
  ggtitle("Vectors x' and y'") + 
  xlab("x") + 
  ylab("y") + 
  theme_minimal()
```
...
# Cosine of the angle
cos_theta <- dot_product / (length_x * length_y)
...
proj_scalar <- dot_product / sum(x^2)
projection <- proj_scalar * x
projection
```

c) Since x_bar = 3 and y_bar = 1, graph [5-3, 1-3, 3-3] = [2, -2, 0] and [-1-1, 3-1 1-1] = [-2, 2, 0]
```{r}
# Define the adjusted vectors
adjusted_vec1 <- c(2, -2, 0)
adjusted_vec2 <- c(-2, 2, 0)

# Data frame to plot vectors
adjusted_vector_data <- data.frame(
  x = c(0, 0), 
  y = c(0, 0), 
  xend = c(adjusted_vec1[1], adjusted_vec2[1]), 
  yend = c(adjusted_vec1[2], adjusted_vec2[2])
)

# Plot the vectors
ggplot(adjusted_vector_data, aes(x = x, y = y)) + 
  geom_segment(aes(xend = xend, yend = yend), arrow = arrow(length = unit(0.3, "cm"))) + 
  xlim(-3, 3) + 
  ylim(-3, 3) +
  ggtitle("Adjusted Vectors") + 
  xlab("x") + 
  ylab("y") + 
  theme_minimal()
```
